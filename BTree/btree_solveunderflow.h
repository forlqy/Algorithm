...
void BTree<T>::solveUnderflow(BTNodePosi<T> v){
    while(((_m + 1) >> 1) > v->child.size()){//除非当前节点并未下溢
        BTNodePosi<T> p = v->parent;
        if(!p){//递归基：已到根节点，没有孩子的下限
            if(!v->key.size() && v->child[0]){
                //但倘若作为树根的v已不含关键码，却有(唯一的)非空的孩子，则
                _root = v->child[0]; _root->parent = nullptr;//此节点可被跳过
        ...
        //另外，在实现了孩子指针的判等器后，也可直接调用Vector::find()定位
      //情况1:向左兄弟借关键码
      ...
      //2：向右兄弟借关键码
        if(p->child.size() - 1 > r){//若v不是p的最后一个孩子，则
            ...
            if((_m + 1) / 2 < rs->child.size()){//若该兄弟足够"胖"，则
                ...
                return;//至此，通过左旋完成下溢处理
            }
        }//至此，右兄弟要么为空，要么太"瘦"
      //3：左，右兄弟要么为空(不可能同时)，要么都太"瘦"-------合并
        if(0 < r){//与左兄弟合并
            BTNodePosi<T> ls = p->child[r - 1];//左兄弟必存在
            ...
            BTNodePosi<T> rs = p->child[r + 1];//右必存在
        ...
        v = p;//上升一层，如有必要继续旋转或合并------至多O(logn)层
    }//while
}//solveUnderflow